...
public class ReactionView extends View {
    ...
    private void beforeAnimateBeginning() {
        ...
        for (int i = 0; i < emotions.length; i++) {
            ...
            emotions[i].currentX = i == 0 ? Board.BOARD_X + CommonDimen.DIVIDE : emotions[i - 1].currentX + emotions[i - 1].currentSize + CommonDimen.DIVIDE;
        }
    }
    ...
    private void calculateCoordinateX() {
        emotions[0].currentX = Board.BOARD_X + CommonDimen.DIVIDE;
        emotions[emotions.length - 1].currentX = Board.BOARD_X + Board.BOARD_WIDTH - CommonDimen.DIVIDE - emotions[emotions.length - 1].currentSize;

        for (int i = 1; i < currentPosition; i++) {
            emotions[i].currentX = emotions[i - 1].currentX + emotions[i - 1].currentSize + CommonDimen.DIVIDE;
        }

        for (int i = emotions.length - 2; i > currentPosition; i--) {
            emotions[i].currentX = emotions[i + 1].currentX - emotions[i].currentSize - CommonDimen.DIVIDE;
        }

        if (currentPosition != 0 && currentPosition != emotions.length - 1) {
            if (currentPosition <= (emotions.length / 2 - 1)) {
                emotions[currentPosition].currentX = emotions[currentPosition - 1].currentX + emotions[currentPosition - 1].currentSize + CommonDimen.DIVIDE;
            } else {
                emotions[currentPosition].currentX = emotions[currentPosition + 1].currentX - emotions[currentPosition].currentSize - CommonDimen.DIVIDE;
            }
        }
    }
